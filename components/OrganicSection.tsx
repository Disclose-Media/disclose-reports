...
import type { WindsorOrganicResult } from '@/lib/windsor'
...
function KpiTile({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="flex flex-col px-5 py-4">
      ...
      {sub && (
        <p className="text-[10px] text-[#555555] mt-1" style={{ fontFamily: 'Inter, sans-serif' }}>
          {sub}
        </p>
      )}
    </div>
...
function TrendChart({ daily }: { daily: WindsorOrganicResult['daily'] }) {
      ...
      chartRef.current = new Chart(canvasRef.current!, {
        ...
        data: {
          ...
          datasets: [
            ...
            {
              label: 'Engagements',
              data: daily.map((d) => d.engagements),
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              ...
              callbacks: {
                label: (ctx) => ` ${ctx.dataset.label}: ${Number(ctx.raw).toLocaleString('en-NZ')}`,
              },
            },
          },
          scales: {
...
export function OrganicSection({ windsorOrganic, igInsights = null }: Props) {
  ...
  return (
    <div className="mb-8">
        ...
        <div className="border-b border-[#1E1E1E] px-6 py-3 flex items-center justify-between">
          ...
          {fb.totalPageLikes > 0 && (
            <span className="text-[10px] text-[#555555]" style={{ fontFamily: 'Inter, sans-serif' }}>
              {fmt(fb.totalPageLikes)} page likes
            </span>
          )}
        </div>
      ...
      {/* Platform cards */}
      <div className={`grid gap-4 ${hasBothPlatforms ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 max-w-sm'}`}>
        ...
        <div className="bg-white border border-[#E8E4DC] rounded-[8px] p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span
              className="text-[9px] px-2 py-1 rounded-full border border-[#E8E4DC] text-[#888888]"
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2.5">
                <svg width="18" height="18" viewBox="0 0 24 24">
                  <defs>
                    <linearGradient id="ig-grad-org" x1="0%" y1="100%" x2="100%" y2="0%">
              ...
              {igInsights.totalFollowers > 0 && (
                <span className="text-[9px] px-2 py-1 rounded-full border border-[#E8E4DC] text-[#888888]" style={{ fontFamily: 'Inter, sans-serif' }}>
                  {fmt(igInsights.totalFollowers)} followers
                </span>
              )}
            </div>
            <MetricRow label="Total Followers" value={igInsights.totalFollowers} />
            <MetricRow label="Impressions" value={igInsights.views} />
            <MetricRow label="Reach" value={igInsights.reach} />
            <MetricRow label="Profile Visits" value={igInsights.profileVisits} gold />
            <MetricRow label="New Follows" value={igInsights.follows} green />
          </div>
        )}
      </div>
    </div>
